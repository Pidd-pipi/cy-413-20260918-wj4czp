...
- **七天回收站**：移除后情绪记录立即退出列表、筛选和本周曲线；七天内可在回收站按原日期还原（日期、等级、标签、备注不变），过期自动清除；重复移除、重复还原结果保持不变，且只显示当前账号的记录。
...
| GET | `/api/v1/moods/trash` | 七天回收站（仅当前账号，自动清除过期记录） |
| PUT / DELETE | `/api/v1/moods/:id` | 修改/移入回收站（软删除，重复移除结果不变） |
| POST | `/api/v1/moods/:id/restore` | 从回收站按原日期还原（重复还原结果不变） |
